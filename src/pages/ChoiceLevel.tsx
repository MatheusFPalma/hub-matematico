    ...
    return (
        <Grid container sx={{ alignItems: 'center', textAlign: 'center', flexDirection: 'column', marginTop: '60px' }} >
            ...
            <Grid item sx={{ marginTop: '30px', display: 'flex', flexDirection: 'column', gap: "20px", justifyContent: 'center', alignItems: "center" }}>
                <button onClick={() => getLevel('Fácil')} className="buttonChallenger" value={currentLevel}>Fácil <img src={levelEasy} alt='levelEasy' /></button>
                <button onClick={() => getLevel('Médio')} className="buttonChallenger" value={currentLevel}>Médio <img src={levelMiddle} alt='levelMiddle' /></button>
                <button onClick={() => getLevel('Difícil')} className="buttonChallenger" value={currentLevel}>Difícil <img src={levelHard} alt='levelHard' /></button>
            </Grid>
        </Grid>
    )
}
